...
// -----------------------------------------------------------------------------
#define CXXFORTH_ENC wxConvUTF8
//#define V   (void)
...
// CMOVE ( c-addr1 c-addr2 u -- ) источник назначение сколько; откуда куда сколько
void cMove() {
...
// CMOVE> ( c-addr1 c-addr2 u -- = c-addr1 this source, c-addr2 this destination )
// from to ( откуда куда сколько -- ) c-addr это указатели
void cMoveUp() {
    REQUIRE_DSTACK_DEPTH(3, "CMOVE>");
    auto length = SIZE_T(*dTop); pop();
...
cout << "len1 " << len1 << ", len2 " << len2 << ", res " << cmpResult << endl;
//*dTop = static_cast<Cell>(cmpResult);
...
The Golyy Part
---------------------
Next I will define Golly primitives.
Все примитивы Голли имеют тип static void, т.е. не возвращают ни каких значений, а вычисленные параметры и параметры необходимые для работы берут со стека и возвращают в стек.

****/
// #define CXXFORTH_ENC wxConvUTF8
static bool aborted = false;    // stop the current script?

static void cxxfCheckEvents()
{
    ...
    if (aborted) {
	    ...
	    cout << "cxxf:checkexents error" << endl;
		throw AbortException("error check event");

    }
}

static void cxxf_rotate(){
  cxxfCheckEvents();
  int direction = 1;
  if (viewptr->SelectionExists()){
	...
	throw AbortException("rotate error: no selection");
  }

}

static void cxxf_update(){
  cxxfCheckEvents();
  GSF_update();
}

static void cxxf_getrect(){ // getrect ( -- x y wd bt flag = true -1 | flag =  false 0 )
  cxxfCheckEvents();
  if (!currlayer->algo->isEmpty()){
	bigint top, left, bottom, right;
	currlayer->algo->findedges(&top, &left, &bottom, &right);
	if (viewptr->OutsideLimits(top, left, bottom, right)){
	  throw AbortException("getrect error: pattern is too big.");
	}
	int x = left.toint();
	int y = top.toint();
	int wd = right.toint() - x + 1;
	int bt = bottom.toint() - y + 1;
	REQUIRE_DSTACK_AVAILABLE(5, "GETRECT");
	push( x );
	push( y );
	push( wd );
	push( bt );
	push( -1 ); // is true result
  }else{
	REQUIRE_DSTACK_AVAILABLE(1, "GETRECT");
	push( 0 );
  }
}

static void cxxf_numstates(){ // ( -- n = numstates )
  REQUIRE_DSTACK_AVAILABLE(1, "NUMSTATES");
...
/****
 getcell (y x -- state)
****/
static void cxxf_getcell(){// ( y x -- n = state )
  cxxfCheckEvents();
  REQUIRE_DSTACK_DEPTH(2, "GETCELL");
  auto x = *dTop; pop();
  auto y = *dTop; pop();
  ...
  if (err){
	...
	REQUIRE_DSTACK_AVAILABLE( 1, "GETCELL" );
	push( currlayer->algo->getcell(x, y));
  }
}

static void cxxf_setcell(){ // ( x1 y1  state -- -  )
  cxxfCheckEvents();
  const char* err;
  REQUIRE_DSTACK_DEPTH(3, "SETCELL");
  auto setcell_state = *dTop; pop();
  auto y = *dTop; pop();
  auto x = *dTop; pop();
...
void cxxfsprivet(){
  cxxfCheckEvents();
  cout << "from cxxforth privet" << endl;
}

/****
static void cxxf_getevent(void)
{
	cxxfCheckEvents();
    bool get = true;
//    if (lua_gettop(L) > 0) get = CheckBoolean(L, 1);
    ...
    s =  (const char*)event.mb_str(CXXFORTH_ENC);
    
}
****/

static void cxxf_show(){ // ( c-addr n -- )
  cxxfCheckEvents();

//  const char* msg = (char*)S0;
  REQUIRE_DSTACK_DEPTH(2, "SHOW");
  auto length = SIZE_T(*dTop); pop();
  auto caddr = CHARPTR(*dTop); pop();
//    throw AbortException(caddr, count);
  inscript = false;
  // wxPuts(msg); // 
  statusptr->DisplayMessage(wxString(string(caddr, length))); //, ATL_ENC));
  inscript = true;
...
/****
Что-то тут не работает. Выводит какую-то дрянь
при вызове 
getcursor type 
печатает какую-то ерунду
хотя в самом s  содержится правильное имя курсора

****/
static void cxxf_getcursor(){ // ( -- c-addr u )
  cxxfCheckEvents();
  string s = CursorToString(currlayer->curs);
  cout << "cursor " << s << s.data() << endl; // this ok
//  push(CELL(s.data()));
  const char* sptr;
  auto sCAddr = CADDR(const_cast<char*>(s.data()));
  sptr = s.data();
  printf("addr: %d, %d", sptr, &sptr);
  REQUIRE_DSTACK_AVAILABLE(2, "GETCURSOR");
  push(CELL(sCAddr));
  //push (CELL(sptr));
  push(s.length());
...
static void cxxf_setcursor(){ // ( c-addr u -- )
  cxxfCheckEvents();
  REQUIRE_DSTACK_DEPTH(2, "SETCURSOR");
  auto length = SIZE_T(*dTop); pop();
  auto caddr = CHARPTR(*dTop); pop();
//  std::string newcursor = string( caddr, length);
  std::string oldcursor = CursorToString(currlayer->curs);
  cout << "oldcursor" << oldcursor << endl;
  const char* newcrs = caddr;
  wxCursor* cursptr = StringToCursor( newcrs );
  if ( cursptr ){
	viewptr->SetCursorMode(cursptr);
	mainptr->UpdateUserInterface();
  }else{
	throw AbortException("setcursor error: unknown cursor string.");
  }
}

static void cxxf_getoption(){ // ( c-addr u -- n )
  cxxfCheckEvents();
  const char* optname;
  REQUIRE_DSTACK_DEPTH(2, "GETOPTION");
  auto length = SIZE_T(*dTop); pop();
  auto caddr = CHARPTR(*dTop); pop();
  optname = caddr;
  int optval;
  if (!GSF_getoption(optname, &optval)) {
	throw AbortException( "getoption error: unknown option.");
  }
  REQUIRE_DSTACK_AVAILABLE(1, "GETOPTION");
  push(optval);
}

/***
Эта версия getevent сделана по аналогии как в атласте
и это работает
***/
static void cxxf_getevent(){ //( c-addr u1 -- c-addr u2 )
  cxxfCheckEvents();
  REQUIRE_DSTACK_DEPTH(2, "GETEVENT");
  auto length = SIZE_T(*dTop); pop();
  auto caddr = CHARPTR(*dTop); 
  bool get = true;
  wxString event;
  ...
  ss =  (const char*)event.mb_str(CXXFORTH_ENC);
  cout << "event:" << ss.data() << ", length " << ss.length() << endl;
  REQUIRE_DSTACK_DEPTH(1, "GETEVENT");
  strcpy(caddr, ss.data());
  REQUIRE_DSTACK_AVAILABLE(1, "GETEVENT");
  push(ss.length());
}
...
string ss; // = strbuf;
const char* cstr;
char cstrar[] =  {"'пример строки для буфера обмена о событии длиной около 40 символов"};
//cstrar = {"'пример строки для буфера обмена о событии длиной около 40 символов"};
  GSF_getevent(event, get ? 1 : 0);
...
if (!showstatus) mainptr->ToggleStatusBar();
  ...
  cout << "getevent: " << s << endl;
cout << "string:" << ss.data() << endl;

std::unique_ptr<char []> cstr1 = std::make_unique<char[]>(ss.size() + 1);

std::copy(ss.data(), ss.data() + ss.size(), &cstr1[0]);
cstr1[ss.size()] = '\0'; // A null-terminator needs to be added

strcpy(strbuf, ss.data());
s = (const char*)ss.data();
// printf("printf: %d, %s", strbuf, *strbuf);
//  push (CELL(s.data()));
//  push(s.length());
//push(CELL(ss.data()));
//push(CELL(&strbuf[0]));
...
/***
Принимает строку и в нее же пишет и отдает
***/
static void cxxf_getxy (){
  cxxfCheckEvents();
  REQUIRE_DSTACK_DEPTH(2, "GETXY");
  auto length = SIZE_T(*dTop); pop();
  auto caddr = CHARPTR(*dTop); 
  bool get = true;
  wxString event;
  ...
  ss =  (const char*)mousepos.mb_str(CXXFORTH_ENC);
  cout << "getxy:" << ss.data() << ", length " << ss.length() << endl;
  REQUIRE_DSTACK_DEPTH(1, "GETXY");
  strcpy(caddr, ss.data());
  REQUIRE_DSTACK_AVAILABLE(2, "GETXY");
  push(ss.length());
}

/***
Принимает строку событие и исполняет это событие
***/
static void cxxf_doevent(){
  cxxfCheckEvents();
  REQUIRE_DSTACK_DEPTH(2, "DOEVENT");
  auto length = SIZE_T(*dTop); pop();
  auto caddr = CHARPTR(*dTop); 
  bool get = true;
  wxString wxevent;
  string ss;  
  const char* event = caddr;
  cout << event << endl;

  if (event[0]){
	inscript = false;
	const char* err = GSF_doevent( wxString( event, CXXFORTH_ENC));
	inscript = true;
	cout << "doevent err" << err << endl;
  }
}

void cxxf_strncmp() { // ( c-addr1 u1 c-addr2 u2 n -- f = flag d, 0 , -d )
    REQUIRE_DSTACK_DEPTH(4, "STRNCMP");
auto len_n = SIZE_T(*dTop); pop();
    auto len2 = SIZE_T(*dTop); pop();
    auto caddr2 = CHARPTR(*dTop); pop();
    auto len1 = SIZE_T(*dTop); pop();
    auto caddr1 = CHARPTR(*dTop);
	...
	auto minLen = std::min(len1, len2);
    auto cmpResult = std::strncmp(caddr1, caddr2, len_n);
	cout << "strncomp: " << cmpResult << endl;
	*dTop = static_cast<Cell>(cmpResult);
...
ищет в строке c-addr1 u1 вхждение строки c-addr2 u2
Если находит возвращает позицию первого вхождения
Если не находит возвращает -1
***/
void cxxf_strfind(){ // ( c-addr1 u1 c-addr2 u2 -- n = pos )
  string ss;
  string delim;
  REQUIRE_DSTACK_DEPTH(4, "STRFIND");
  auto length_delim = SIZE_T(*dTop); pop();
  auto caddr_delim = CHARPTR(*dTop); pop(); 
  delim = string(caddr_delim);
//  if ( delim.find(" ") != string::npos ){
//	cout << "delim =" << delim << "пробел" << endl;
//  }else{
//	pop();
//	*dTop = -1;
//	cout << "delim not space" << endl;
//	return;
//  }
  auto length_str = SIZE_T(*dTop); pop();
//  auto caddr_str = CHARPTR(*dTop); 
  ss = string(CHARPTR(*dTop)); 
  cout << "str " << ss << endl;
  // ss = caddr_str;
  if (ss.find(delim) != string::npos){
	int pos = ss.find( delim );
	cout << "strfind: " << ss.data() << ", delim: " << delim.data() << ", pos: " << pos << endl;

	*dTop = pos;
  }else{ // если ничего не найдено или другая ошибка
	*dTop = -1;
  }
}
...
void defineGollyPrimitives() {
    static struct {
        ...
        {"ROTATE",             cxxf_rotate},
		{ "UPDATE",           cxxf_update },
		{ "GETRECT",           cxxf_getrect },
		{ "NUMSTATES",           cxxf_numstates },
		{ "GETCELL",           cxxf_getcell },
		{ "SETCELL",           cxxf_setcell },
		{ "SHOW",           cxxf_show },
		{ "GETCURSOR",           cxxf_getcursor },
		{ "SETCURSOR",           cxxf_setcursor },
		{ "GETOPTION",           cxxf_getoption },
		{ "GETEVENT",           cxxf_getevent },
		{ "DOEVENT",           cxxf_doevent },
		{ "GETXY",           cxxf_getxy },
		{ "STRFIND",           cxxf_strfind },
		{ "STRNCMP",           cxxf_strncmp },
    };
    for (auto& w: gollyPrimitives) {
        definePrimitive(w.name, w.code);
    }
}
/***
...
This equal
begin-enum enum apple enum banana enum cherry end-enum
...
/****
Это within взято из стандарта 2012. 
Потребовалось для работы FFL
****/
...
/***
Это не работает. Нет +loop а в нем нет jmp
Где взять jmp неизвестно
...
char **CxxForthAddPtr (char **pp, int size, char *str);
...
void RunCxxForthScript(const wxString& filepath){
  ...
  FILE* fp;
    ...
    try {
        commandLineArgCount = 0; //static_cast<size_t>(argc);
        commandLineArgVector = nullptr; //argv;
		int argc0 = 0;
		char **argv0 = 0;

		char* pchr = (const_cast<char*>((const char*)filepath.mb_str()));

		argv0 = CxxForthAddPtr( argv0, argc0, "golly");
		argc0++;
		commandLineArgCount++;
		argv0 = CxxForthAddPtr( argv0, argc0, pchr);
		argc0++;
		commandLineArgCount++;

		commandLineArgVector = (const char**)argv0;
